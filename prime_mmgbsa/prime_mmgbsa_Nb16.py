...
for cluster in Nb16_cluster:
        for i in range(1,5):
            filename = f"HER2_Nb16_cluster{cluster}_{i}_prime_mmgbsa-out.csv" 
            file_path = find_file_in_directory(directory, filename)
            if file_path: continue
            
            script_content = f"""#!/bin/bash
        $SCHRODINGER/prime_mmgbsa  ../HER2_Nb16/prep_files/HER2_Nb16_cluster{cluster}_{i}.maegz -csv_output=yes -ligand="chain. {Nb_chain}" -jobname=HER2_Nb16_cluster{cluster}_{i}_prime_mmgbsa -job_type=ENERGY
        """
            
            script_filename = "mmgbsa_script_Nb16.sh"
