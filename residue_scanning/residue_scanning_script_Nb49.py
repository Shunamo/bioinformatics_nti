...
for cluster in Nb49_cluster:
    for i in range(1,5):
        filename = f"residue_scanning_HER2_Nb49_cluster{cluster}_{i}-out.maegz"
        ...
        script =f"""#!/bin/bash
"$SCHRODINGER/run" residue_scanning_backend.py -fast -jobname residue_scanning_HER2_Nb49_cluster{cluster}_{i} -res_file residue_scanning_HER2_Nb49-mutations.txt -refine_mut prime_residue -calc hydropathy,rotatable,vdw_surf_comp,sasa_polar,sasa_nonpolar,sasa_total -dist 0.00 ../HER2_Nb49/prep_files/HER2_Nb49_cluster{cluster}_{i}.maegz -receptor_asl 'NOT (chain.n B)' -add_res_scan_wam -HOST localhost:20 -TMPLAUNCHDIR
"""
